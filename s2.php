...
$opts = array('officialserver'=>0,'minplayers'=>0,'notfull'=>0,'officialmap'=>0,'sort'=>'name');
foreach($opts as $o=>$v) {
	if (isset($_GET[$o]))
		$$o = $_GET[$o];
	else
...
<input type="checkbox" name="officialserver"<?ch($officialserver);?>> Show official servers.
...
while (($dat = $db->fetchobject($res))) {
	...
	for ($i = 0; $i < $mps; $i++) {
		if ($i < $cps) {
			...
			echo "<img src=\"img/aligng.png\">";
		} else {
			echo "<img src=\"img/alignn.png\">";
		}
	}
}
