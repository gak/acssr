...
echo "\tnode[shape=\"box\", width=.01, height=.01, fontname=\"Vera\", fontsize=8.0];\n";
echo "\tedge[arrowsize=0.3];\n";

while ($dat = $db->fetchObject($resf)) {
	echo "\tid" . $dat->a . " -> id" . $dat->b . " [color=royalblue, len=2];\n";
...
while ($dat = $db->fetchObject($resv)) {
	...
	if ($dat->moo < 0) {
		$col = "firebrick2";
		echo "\tid" . $dat->b . " -> id" . $dat->a . " [color=$col, len=4];\n";
	} else {
		$col = "forestgreen";
		echo "\tid" . $dat->a . " -> id" . $dat->b . " [color=$col, len=3];\n";
	}
