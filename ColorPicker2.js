...
function ColorPicker() {
	...
	if (windowMode) {
		...
		cp_contents += "<style>\n";
		cp_contents += '@import "cp.css";\n';
		cp_contents += "</style>\n";
		cp_contents += "<BODY>";
		}
	cp_contents += "<TABLE width=100%>";
	var use_highlight = (document.getElementById || document.all)?true:false;
	for (var i=0; i<total; i++) {
		if ((i % width) == 0) { cp_contents += "<TR>"; }
		if (use_highlight) { var mo = 'onMouseOver="'+windowRef+'ColorPicker_highlightColor(\''+colors[i]+'\',window.document)"'; }
		else { mo = ""; }
		cp_contents += '<TD BGCOLOR="'+colors[i]+'" onClick="'+windowRef+'ColorPicker_pickColor(\''+colors[i]+'\','+windowRef+'window.popupWindowObjects['+cp.index+']);return false;"><FONT SIZE="-3"><A HREF="#" onClick="'+windowRef+'ColorPicker_pickColor(\''+colors[i]+'\','+windowRef+'window.popupWindowObjects['+cp.index+']);return false;" '+mo+' STYLE="text-decoration:none;">&nbsp;&nbsp;&nbsp;</A></FONT></TD>';
	...
	if (document.getElementById) {
		...
		cp_contents += "<TR><TD COLSPAN='"+width1+"' ID='colorPickerSelectedColor'>&nbsp;</TD><TD COLSPAN='"+width2+"' ID='colorPickerSelectedColorValue'>FFFFFF</TD></TR>";
		}
	cp_contents += "</TABLE>";
	if (windowMode) {
